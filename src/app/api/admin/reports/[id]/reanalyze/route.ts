  ...
  try {
    ...
    if (!report.reportTranscript || report.reportTranscript.trim().length === 0) {
      return NextResponse.json(
        { error: 'Report transcript is empty. Extract text or write it manually first.' },
        { status: 400 }
      );
    }
    ...
    // Save AI Log and update Report inside transaction
    const updatedReport = await prisma.$transaction(async (tx) => {
      ...
      if (availableDoctor) {
        ...
        // If no doctor available in new category, keep existing assignment OR set to null
        // Let's set it to null and PROCESSING status since the category changed and no doctor matches
