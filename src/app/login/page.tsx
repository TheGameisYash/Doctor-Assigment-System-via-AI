  ...
  const handleDemoLogin = async () => {
    ...
    try {
      const res = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ email: 'patient@healthcare.com', password: 'patient123' }),
      });
    ...
    <div className="relative min-h-screen flex flex-col items-center justify-center bg-slate-950 px-4 py-12 text-slate-100">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 text-sm">Patient Login Portal</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full rounded-lg border border-slate-700 bg-slate-950 py-2.5 pl-10 pr-3.5 text-slate-100 placeholder-slate-500 shadow-sm focus:border-teal-500 focus:outline-none focus:ring-1 focus:ring-teal-500 text-sm transition-colors"
            ...
            <div className="space-y-4">
              <button
                type="submit"
                disabled={loading}
                className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-teal-500 hover:bg-teal-400 text-slate-950 py-3 text-sm font-bold shadow transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
        ...
        {/* Alternative Portals */}
        <div className="mt-8 flex justify-center gap-6 text-xs text-slate-500 border-t border-slate-900 pt-6">
          <Link href="/doctor/login" className="hover:text-slate-300 transition-colors">
            Doctor Portal
          </Link>
          <span>&bull;</span>
